...
```text
Project-PayFlow-API/
├── main.py                 # 啟動 Event Worker 
├── core/                   #
│   ├── config.py           #
│   ├── database.py         #)
│   ├── event_bus.py        # EventBus 類別實作
│   └── email.py            # Email 寄送 (aiosmtplib 封裝)
├── features/              
│   ├── products/
│   │   ├── router.py       # API: POST /products/
│   │   ├── service.py      
│   │   └── models.py       
│   ├── orders/
│   │   ├── router.py       # API: POST /orders/, GET /orders/{id}/status
│   │   ├── service.py      
│   │   └── models.py       
│   └── payments/           
│       ├── router.py       # API: POST /webhooks/ecpay
│       ├── ecpay_service.py
│       └── event_handlers.py # 訂閱 EventBus：收到付款成功後 -> 呼叫寄信或扣庫存
└── requirements.txt
